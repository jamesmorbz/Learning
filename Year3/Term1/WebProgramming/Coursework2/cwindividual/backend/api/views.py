...
def elements(request: WSGIRequest):
    if request.method == 'GET':
        ...
        data = [{'id': product.pk, 'name': product.name, 'description': product.description, 'price': product.price, 'is_available': product.is_available} for product in products]
        ...
        print(f"got data going to create a new product {name} {description} {price} {is_available}")
        if name is not None and description is not None and price is not None and is_available is not None:
...
def update_element(request: WSGIRequest, pk):
    if request.method == "PUT":
        ...
        if name and description and price and is_available:
            ...
            return JsonResponse({'message': 'Incomplete data. Please provide name, description, price, and is_available.'}, status=HttpResponse.HTTP_400_BAD_REQUEST)
